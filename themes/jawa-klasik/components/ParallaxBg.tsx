...
/* Layer 1 - Janur (Daun Kelapa Muda) */
function JanurLayer() {
  const ref = useScrollParallax(0.05);

  const JanurSvg = ({ size, rotate = 0 }: { size: number; rotate?: number }) => (
    <svg viewBox="0 0 80 120" width={size} height={size * 1.5} style={{ transform: `rotate(${rotate}deg)` }}>
      <path d="M40,5 C40,5 40,115 40,115" fill="none" stroke={JAWA_COLORS.emas} strokeWidth="2" />
  ...
  return (
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      <div style={{ position: "absolute", top: "3%", left: "2%", opacity: 0.35 }}><JanurSvg size={70} rotate={-20} /></div>
      <div style={{ position: "absolute", top: "3%", right: "2%", opacity: 0.35 }}><JanurSvg size={70} rotate={20} /></div>
      <div style={{ position: "absolute", bottom: "5%", left: "3%", opacity: 0.3 }}><JanurSvg size={60} rotate={-15} /></div>
      <div style={{ position: "absolute", bottom: "5%", right: "3%", opacity: 0.3 }}><JanurSvg size={60} rotate={15} /></div>
    </div>
  );
}

/* Layer 2 - Batik Parang Pattern */
function BatikParangLayer() {
  ...
  const ParangSvg = ({ size }: { size: number }) => (
    <svg viewBox="0 0 60 80" width={size} height={size * 1.33}>
      <path d="M5,80 C15,70 25,60 20,50 C15,40 25,30 35,20 C45,10 55,0 55,0" fill="none" stroke={JAWA_COLORS.sogan} strokeWidth="3" />
      <path d="M15,80 C25,70 35,60 30,50 C25,40 35,30 45,20 C55,10 60,5 60,0" fill="none" stroke={JAWA_COLORS.emas} strokeWidth="2" />
  ...
  const positions = [
    { left: "3%", top: "20%", size: 55, opacity: 0.3 },
    { left: "92%", top: "25%", size: 50, opacity: 0.28 },
    { left: "5%", top: "50%", size: 52, opacity: 0.28 },
    { left: "90%", top: "55%", size: 55, opacity: 0.3 },
    { left: "4%", top: "75%", size: 48, opacity: 0.25 },
    { left: "91%", top: "80%", size: 50, opacity: 0.28 },
  ];

  return (
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      {positions.map((p, i) => (
        <div key={i} style={{ position: "absolute", left: p.left, top: p.top, opacity: p.opacity, transform: i % 2 === 0 ? "none" : "scaleX(-1)" }}>
...
function GununganLayer() {
  ...
  const GununganSvg = ({ size, opacity }: { size: number; opacity: number }) => (
    <svg viewBox="0 0 100 180" width={size} height={size * 1.8} style={{ opacity }}>
      <path d="M50,5 C50,5 15,50 10,90 C5,130 15,155 50,175 C85,155 95,130 90,90 C85,50 50,5 50,5 Z" fill={JAWA_COLORS.emas} stroke={JAWA_COLORS.sogan} strokeWidth="2" />
      <path d="M50,15 C50,15 25,50 22,85 C19,115 28,138 50,155 C72,138 81,115 78,85 C75,50 50,15 50,15 Z" fill="none" stroke={JAWA_COLORS.sogan} strokeWidth="1.5" />
      <path d="M50,30 C50,30 35,55 33,80 C31,105 38,120 50,132 C62,120 69,105 67,80 C65,55 50,30 50,30 Z" fill="none" stroke={JAWA_COLORS.merahTua} strokeWidth="1" />
      <circle cx="50" cy="25" r="10" fill={JAWA_COLORS.sogan} />
      <circle cx="50" cy="25" r="6" fill={JAWA_COLORS.emas} />
      {[0, 45, 90, 135, 180, 225, 270, 315].map((angle, i) => {
        const rad = (angle * Math.PI) / 180;
        return <line key={i} x1={50 + 12 * Math.cos(rad)} y1={25 + 12 * Math.sin(rad)} x2={50 + 18 * Math.cos(rad)} y2={25 + 18 * Math.sin(rad)} stroke={JAWA_COLORS.sogan} strokeWidth="2" />;
      })}
  ...
  return (
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      <div style={{ position: "absolute", top: "5%", left: "5%" }}><GununganSvg size={80} opacity={0.35} /></div>
      <div style={{ position: "absolute", top: "25%", right: "4%", transform: "scaleX(-1)" }}><GununganSvg size={70} opacity={0.3} /></div>
      <div style={{ position: "absolute", bottom: "8%", left: "8%" }}><GununganSvg size={65} opacity={0.28} /></div>
      <div style={{ position: "absolute", bottom: "15%", right: "6%", transform: "scaleX(-1)" }}><GununganSvg size={60} opacity={0.25} /></div>
    </div>
  );
}

/* Layer 4 - Wayang Characters */
function WayangLayer() {
  const ref = useScrollParallax(0.15);

  const WayangSvg = ({ flip = false, size = 100 }: { flip?: boolean; size?: number }) => (
    <svg viewBox="0 0 60 120" width={size * 0.5} height={size} style={{ transform: flip ? "scaleX(-1)" : "none" }}>
  ...
  return (
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      <div style={{ position: "absolute", top: "15%", left: "10%", opacity: 0.35 }}><WayangSvg size={120} /></div>
      <div style={{ position: "absolute", top: "20%", right: "8%", opacity: 0.3 }}><WayangSvg flip size={110} /></div>
      <div style={{ position: "absolute", bottom: "18%", right: "12%", opacity: 0.32 }}><WayangSvg size={100} /></div>
      <div style={{ position: "absolute", bottom: "25%", left: "8%", opacity: 0.28 }}><WayangSvg flip size={95} /></div>
      <div style={{ position: "absolute", top: "45%", left: "5%", opacity: 0.25 }}><WayangSvg size={85} /></div>
      <div style={{ position: "absolute", top: "50%", right: "5%", opacity: 0.25 }}><WayangSvg flip size={85} /></div>
    </div>
  );
}

/* Layer 5 - Kawung Batik Pattern */
function KawungLayer() {
  const ref = useScrollParallax(0.2);

  const kawungPositions = [
    { left: "15%", top: "30%", size: 50, opacity: 0.35 },
    { left: "78%", top: "22%", size: 45, opacity: 0.32 },
    { left: "22%", top: "55%", size: 55, opacity: 0.38 },
    { left: "72%", top: "58%", size: 48, opacity: 0.33 },
    { left: "45%", top: "75%", size: 52, opacity: 0.35 },
    { left: "10%", top: "72%", size: 42, opacity: 0.3 },
    { left: "85%", top: "68%", size: 48, opacity: 0.32 },
    { left: "35%", top: "40%", size: 40, opacity: 0.28 },
    { left: "60%", top: "35%", size: 44, opacity: 0.3 },
  ];

  return (
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      {kawungPositions.map((k, i) => (
        <svg key={i} viewBox="0 0 40 40" width={k.size} height={k.size} style={{ position: "absolute", left: k.left, top: k.top, opacity: k.opacity }}>
    ...
    <div ref={ref} style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden", willChange: "transform" }}>
      {particles.map((p, i) => (
        <div key={i} style={{
          ...
          opacity: p.opacity,
          boxShadow: `0 0 ${p.size * 3}px ${JAWA_COLORS.emas}`,
        }} />
      ))}
    </div>
  );
